...
<div class="split">
  ...
  <div class="right-pane">
    ...
    <div class="results-toolbar" id="results-toolbar" style="display:none">
      ...
      <div class="toolbar-sep"></div>
      <button class="nav-btn" id="rank-smart-btn" onclick="toggleRankBySmart()" title="Sort by outcome tier + longest hops (PREFER_DELIVERED: best outcome tier first, then deepest; PREFER_VIOLATIONS: API order)">&#x2195; smart rank: off</button>
      <div class="toolbar-sep"></div>
      ...
      <div class="toolbar-sep"></div>
      <button class="nav-btn" id="copy-summary-btn" onclick="copySummary()" title="Copy path summary to clipboard">&#x2398; copy summary</button>
      <div class="toolbar-sep"></div>
      <button class="nav-btn" id="export-summary-btn" onclick="exportSummary()" title="Download path summary as .txt">&#x21E9; export summary</button>
    </div>
...
function initNavigator(totalHits) {
  ...
  // Smart rank — reset
  rankBySmart = false;
  const smartBtn = document.getElementById('rank-smart-btn');
  smartBtn.textContent      = '↕ smart rank: off';
  smartBtn.style.color      = '';
  smartBtn.style.borderColor = '';
...
function applyFilters() {
  ...
  if (rankByFwCount) {
    ...
    const intent = document.querySelector('input[name="intent"]:checked').value;
    const isViolations = intent === 'PREFER_VIOLATIONS' || intent === 'VIOLATIONS_ONLY';
    if (!isViolations) {
      // PREFER_DELIVERED / default: three-tier sort
      // Tier 1: DELIVERED + any security outcome
      // Tier 2: anything else except DELIVERED_TO_INCORRECT_LOCATION
      // Tier 3: DELIVERED_TO_INCORRECT_LOCATION
      function deliveredTier(p) {
        const fwd = p.forwardingOutcome || '';
        if (fwd === 'DELIVERED') return 1;
        if (fwd === 'DELIVERED_TO_INCORRECT_LOCATION') return 3;
        return 2;
      }
      filteredPaths.sort((a, b) => {
        const ta = deliveredTier(a), tb = deliveredTier(b);
        if (ta !== tb) return ta - tb;
        return (b.hops || []).length - (a.hops || []).length;
      });
    }
    // For violations: leave in API order (already optimal)
  }
...
function toggleRankByHops() {
  ...
  if (rankByHops) {
    rankBySmart = false;
    const sb = document.getElementById('rank-smart-btn');
    sb.textContent = '↕ smart rank: off'; sb.style.color = ''; sb.style.borderColor = '';
  }
...
// ── Smart rank toggle ─────────────────────────────────────────────────────────
function toggleRankBySmart() {
  rankBySmart = !rankBySmart;
  if (rankBySmart) {
    rankByHops = false;
    const hb = document.getElementById('rank-hops-btn');
    hb.textContent = '↕ hops: off'; hb.style.color = ''; hb.style.borderColor = '';
    rankByFwCount = false;
    const fb = document.getElementById('rank-fw-count-btn');
    if (fb) { fb.textContent = '↕ rank by FW count: off'; fb.style.color = ''; fb.style.borderColor = ''; }
  }
  const btn = document.getElementById('rank-smart-btn');
  btn.textContent = `↕ smart rank: ${rankBySmart ? 'on' : 'off'}`;
  btn.style.color = rankBySmart ? 'var(--accent)' : '';
  btn.style.borderColor = rankBySmart ? 'var(--accent)' : '';
  applyFilters();
}
...
function buildSummaryText() {
  ...
  const path      = filteredPaths[currentPathIdx];
  const globalPos = allPaths.indexOf(path) + 1;
  const total     = allPaths.length;
  const srcIp     = document.getElementById('srcIp').value.trim();
  const dstIp     = document.getElementById('dstIp').value.trim();
  const ipProto   = document.getElementById('ipProto').value.trim();
  const dstPort   = document.getElementById('dstPort').value.trim();
  const intent    = document.querySelector('input[name="intent"]:checked').value || 'PREFER_DELIVERED';
  const netIdx    = document.getElementById('network-select').value;
  const netName   = netIdx !== '' ? discoveredNetworks[parseInt(netIdx)].name : '';
  const snapSel   = document.getElementById('snapshot-select');
  const snapLabel = snapSel.value
    ? (snapSel.options[snapSel.selectedIndex]?.text || snapSel.value.slice(-8))
    : 'latestProcessed';
  const fwdOutcome = path.forwardingOutcome || '—';
  const secOutcome = path.securityOutcome   || '—';
  const hops       = path.hops || [];
  const PROTO_MAP  = {'1':'ICMP','6':'TCP','17':'UDP','47':'GRE','50':'ESP','51':'AH','58':'ICMPv6'};
  let searchStr = `f(${srcIp})(ipv4_dst.${dstIp})`;
  if (ipProto) searchStr += `(ip_proto.${PROTO_MAP[ipProto] || ipProto})`;
  if (dstPort) searchStr += `(tp_dst.${dstPort})`;
  searchStr += `m(permit_all)`;
  const paramParts = [searchStr];
  if (intent) paramParts.push(intent);
  const hopLines = hops.map((h, i) => {
    const num  = String(i + 1).padStart(3, ' ');
    const name = (h.deviceName || '(unnamed)').padEnd(40, ' ');
    const type = h.deviceType || '';
  ...
  return [
    ...
    '===================',
    paramParts.join('  |  '),
    `Network: ${netName}  |  Snapshot: ${snapLabel}`,
    `${total} result(s)  |  Showing path ${globalPos} of ${total}`,
    `ForwardingOutcome: ${fwdOutcome}  |  SecurityOutcome: ${secOutcome}`,
    '',
    hopLines,
  ].join('\n');
}
...
function exportSummary() {
  ...
  const srcIp   = document.getElementById('srcIp').value.trim();
  const dstIp   = document.getElementById('dstIp').value.trim();
  const safeSrc = srcIp.replace(/[^a-zA-Z0-9._-]/g, '_');
  const safeDst = dstIp.replace(/[^a-zA-Z0-9._-]/g, '_');
  const blob = new Blob([text], { type: 'text/plain' });
  const url  = URL.createObjectURL(blob);
  const a    = document.createElement('a');
  a.href     = url;
  a.download = `path_summary_${safeSrc}_to_${safeDst}.txt`;
  a.click();
...
function toggleFwRank() {
  ...
  if (rankByFwCount) {
    rankByHops = false; document.getElementById('rank-hops-btn').textContent = '↕ hops: off'; document.getElementById('rank-hops-btn').style.color=''; document.getElementById('rank-hops-btn').style.borderColor='';
    rankBySmart = false; document.getElementById('rank-smart-btn').textContent = '↕ smart rank: off'; document.getElementById('rank-smart-btn').style.color=''; document.getElementById('rank-smart-btn').style.borderColor='';
  }
